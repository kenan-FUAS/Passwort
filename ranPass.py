...
def countdown2():               #Timer für das automatische Abmelden nach x Minuten
        ...
        open_File = open(pfad+"\\PManager\\Zeitlimit\\Zeitlimit.txt","r")   #Aufrufen der Datei, in der die selbstgewählte
        min = int(open_File.read())                                         #Minutenanzahl gespeichert ist
...
def zeitlimit():                #Wählen des Zeitlimits, bis automatisch beendet werden soll
    ...
    if benutzer_input == "y":
        ...
        print("Das Zeitlimit beträgt ab dem nächsten Start der Anwendung "+open_File.read()+" Minute(n).")      #Das aktualisierte Zeitlimit wird angegeben
        open_File.close()
